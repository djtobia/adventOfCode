...
for(let i = 0; i < input.length; i++){
    ...
    const char = splitLine[1].charAt(0);
    if((splitLine[2].charAt(locations[0]-1) === char && splitLine[2].charAt(locations[1]-1) !== char) ||
    (splitLine[2].charAt(locations[0]-1) !== char && splitLine[2].charAt(locations[1]-1) === char)){
        countCorrectPart2++;
    }
}
